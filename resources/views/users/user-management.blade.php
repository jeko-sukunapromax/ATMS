<x-app-layout>
    ...
    <div class="relative bg-indigo-600 rounded-[2.5rem] p-8 sm:p-10 shadow-2xl shadow-indigo-200 overflow-hidden mb-8">
        ...
        <div class="relative z-10 flex items-center justify-between">
            ...
            <a href="{{ route('users.create') }}" class="group relative inline-flex items-center justify-center px-8 py-4 text-sm font-black tracking-[0.15em] text-indigo-900 bg-white rounded-2xl overflow-hidden transition-all hover:scale-105 hover:shadow-[0_0_30px_rgba(255,255,255,0.3)]">
    ...
    <div class="bg-white rounded-[2.5rem] p-6 sm:p-10 shadow-2xl shadow-gray-100/50 border border-gray-100">
        ...
        @if(session('warning'))
            <div class="mb-8 p-6 bg-orange-50/80 backdrop-blur-sm border border-orange-100 text-orange-700 rounded-2xl shadow-sm flex items-start gap-4">
                ...
                <div>
                    <p class="font-black uppercase tracking-widest text-xs mb-1">Action Required</p>
                    <p class="text-base font-bold">{{ session('warning') }}</p>
                </div>
            </div>
        @endif
        ...
        <form action="{{ route('users.index') }}" method="GET" class="flex flex-col xl:flex-row gap-4 mb-10 items-center justify-between bg-gray-50/50 p-4 rounded-3xl border border-gray-100 shadow-sm relative z-20">
            ...
            <div class="flex flex-col sm:flex-row gap-4 w-full xl:w-auto flex-1 justify-end">
                <select name="position" onchange="this.form.submit()" class="w-full sm:w-56 px-4 py-4 bg-white border border-transparent rounded-2xl text-sm font-bold text-gray-600 shadow-sm focus:ring-2 focus:ring-indigo-500 hover:border-gray-200 transition-all cursor-pointer truncate">
                ...
                <select name="office" onchange="this.form.submit()" class="w-full sm:w-64 px-4 py-4 bg-white border border-transparent rounded-2xl text-sm font-bold text-gray-600 shadow-sm focus:ring-2 focus:ring-indigo-500 hover:border-gray-200 transition-all cursor-pointer truncate">
                    ...
                    @foreach($offices as $office)
                        <option value="{{ $office->name }}" {{ request('office') == $office->name ? 'selected' : '' }}>{{ \Illuminate\Support\Str::limit($office->name, 30) }}</option>
                    @endforeach
                </select>

                @if(request()->anyFilled(['search', 'position', 'office']))
                    <a href="{{ route('users.index') }}" class="flex items-center justify-center px-6 py-4 bg-red-50 text-red-600 rounded-2xl text-sm font-black uppercase tracking-widest hover:bg-red-100 transition-colors whitespace-nowrap">
                        Clear
                    </a>
                @endif
            </div>
            ...
            @forelse($permanentUsers as $user)
                ...
                <div class="group bg-white border border-gray-100 rounded-[2rem] p-5 lg:p-6 hover:shadow-2xl hover:shadow-indigo-100/40 hover:border-indigo-100 transition-all duration-300 flex flex-col lg:flex-row items-start lg:items-center gap-6 relative overflow-hidden">
                    ...
                    <div class="flex items-center gap-5 lg:w-1/3">
                        ...
                        <div>
                            <h3 class="text-lg font-black text-gray-900 uppercase tracking-tight">{{ $user->name }}</h3>
                            <p class="text-sm font-bold text-gray-500 mt-1">{{ $user->email }}</p>
                        </div>
                    </div>
                    ...
                    <div class="lg:w-1/4 flex flex-col gap-3">
                        <div>
                            <span class="px-4 py-2 bg-{{ $roleColor }}-50 text-{{ $roleColor }}-600 rounded-xl text-xs font-black uppercase tracking-widest inline-flex items-center gap-2 border border-{{ $roleColor }}-100 group-hover:bg-{{ $roleColor }}-100 transition-colors">
                                <span class="w-1.5 h-1.5 rounded-full bg-{{ $roleColor }}-500 animate-pulse"></span>
                                {{ ucfirst($roleLabel) }}
                            </span>
                        </div>
                        <p class="text-sm font-bold text-gray-600 line-clamp-2 leading-relaxed group-hover:text-gray-900 transition-colors" title="{{ $user->position ?: 'No Position Defined' }}">
                    ...
                    <div class="lg:w-1/4">
                        <div class="flex items-center gap-2 mb-1">
                            ...
                            <p class="text-xs font-black text-gray-400 uppercase tracking-widest">Office / Dept</p>
                        </div>
                        <p class="text-base font-bold text-gray-700 line-clamp-2 leading-tight">
                            {{ $user->office ?: 'N/A' }}
                        </p>
                    </div>
